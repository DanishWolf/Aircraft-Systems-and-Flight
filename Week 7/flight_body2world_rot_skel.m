function [ y ] = flight_body2world_rot_skel( X, t, u0, pos_start )
...
%%    % Preallocate our extended state matrix
    ...
    for i = 2:length(X)         
        ...
        %TODO Find Angular/Euler Rates
       % y(i,10) = ;    % Roll rate is not simulated
       % y(i,11) = ;    % dot(pitch) (ref dot(theta) + change)
       % y(i,12) = ;    % Yaw rate is not simulated
        
        % --------------------- IMPORTANT NOTE ----------------------------
        % Strictly speaking, we actually need to rotate the body angular
        ...
        %TODO Find Euler Angles
        y(i,7:9) = y(i-1,7:9) + dt*y(i,10:12)
       % y(i,7) = ...   % Roll (ref phi + change)
       % y(i,8) = ...   % Pitch (ref theta + change)
       % y(i,9) = ...    % Yaw (not simulated)
        
        phi = y(i,7);    % Roll Angle (ref phi + change)        
        theta = y(i,8);  % Pitch Angle (ref pitch + change)
        psi = y(i,9);    % Yaw Angle (ref psi + change)
